...
export default function Inv_unmatch_datpr() {
  ...
  const fetchData = () => {
    if (!cabang) return;
    fetch(`/api/lpd-inv-unmatch?cabang=${cabang}`)
    .then((res) => res.json())
    .then((res) => {
        setData(res.data);
    })
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-4">
            <table className="min-w-full border-collapse">
                ...
                <tbody>
                    {data.length === 0 ? (
                        <tr>
                        <td colSpan={10} rowSpan={10} className="text-center p-4 text-sm text-gray-500">
