...
export default function DATPR_unmatch() {
  ...
  const fetchData = () => {
    if (!cabang) return;
    fetch(`/api/lpd-datpr-unmatch?cabang=${cabang}`)
    .then((res) => res.json())
    .then((res) => {
        setData(res.data);
    })
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet('DAT-PR Unmatch Inv');
    ...
    worksheet.columns = [
        ...
        { header: 'No Seri', key: 'seri' },
        { header: 'Keterangan', key: 'keterangan' },
        { header: 'Invoice Num', key: 'inv_num' },
        { header: 'Tgl Mulai Susut', key: 'tgl_perolehan' },
        { header: 'Harga Perolehan', key: 'harga' },
    ];
    ...
    data.forEach(row => {
        ...
        inv_num: row.inv_num,
        tgl_perolehan: new Date(row.tgl_perolehan),
        harga: row.harga,
        });
    });
    ...
    a.href = url;
    a.download = `DATPR_Unmatch_${cabang}.xlsx`;
    a.click();
      ...
      <div className="h-full bg-white/60 rounded-lg shadow-lg text-gray-800 w-full flex flex-col">
        <div className="relative flex items-center justify-center mb-4 px-4 pt-4">
          <div className="mb-4 text-center">
            <h2 className="text-xl font-bold">Daftar AT/PR Unmatch Invoice</h2>
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-4">
            <table className="min-w-full border-collapse">
                <thead className="bg-gray-100 sticky top-0 z-10 text-sm font-semibold text-center bg-white">
                    ...
                    <th className="p-2 border-b">No Seri</th>
                    <th className="p-2 border-b">Keterangan</th>
                    <th className="p-2 border-b">Invoice Num</th>
                    <th className="p-2 border-b">Tgl Mulai Susut</th>
                    <th className="p-2 border-b">Harga Perolehan</th>
                    </tr>
                </thead>
                <tbody>
                    {data.map((row, index) => (
                    <tr key={index}>
                        ...
                        <td className="p-2 border-b text-sm text-center">{row.seri}</td>
                        <td className="p-2 border-b text-sm text-left">{row.keterangan}</td>
                        <td className="p-2 border-b text-sm text-left">{row.inv_num}</td>
                        <td className="p-2 border-b text-sm text-center">{formatDate(row.tgl_perolehan)}</td>
                        <td className="p-2 border-b text-sm text-right">{formatRupiah(row.harga)}</td>
                    </tr>
                    ))}
                </tbody>
            </table>
